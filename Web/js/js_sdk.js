...
function shareImage(imageurl) {
    ...
    if (type.ios == true) {
        ...
        local_kingkr_obj.share(shareImage)
    }
}
function shareCutImage() {
    ...
    if (type.ios == true) {
        ...
        local_kingkr_obj.share(shareCutImage)
    }
}
...
function qrcode() {
    ...
    type = sdk_judge();
    if (type.ios == true) {
        var message = {'methodName':'qrcoder','callbackMethod':'qrcodeCallback'};
        window.webkit.messageHandlers.local_kingkr_obj.postMessage(message);
    }else{
        local_kingkr_obj.qrcoderWithCallback();
    }
}
function cleanCache() {
    if(!sdk_checkinside())return false;
