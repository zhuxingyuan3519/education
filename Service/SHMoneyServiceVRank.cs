...
namespace Service
{
    ...
    /// 教育网分红系统
    /// </summary>
    public class SHMoneyServiceVRank
    {
        ...
        public static void SetRankPoint(Model.Member member, List<CommonObject> listComm, ref M_Rank rank)
        {
            ...
            {
                //有推荐人的话就往上/往下查找，看看相关层级中的人哪个是在排位表中的
                ...
                string sql = @"SELECT t2.* FROM ( SELECT * FROM dbo.FUN_CountTDMember('" + member.ID + "',0,999)";
                sql += " UNION ALL ";
                sql += "SELECT * FROM dbo.FUN_CountUpperMember('" + member.ID + "',0,999)";
                sql += ") t1  LEFT JOIN dbo.M_Rank t2 ON (t1.Code=t2.MCode OR t1.Code=t2.PMCode) WHERE t1.Code<>'18' AND t2.Code IS NOT NULL order BY t2.RankTime DESC";
                DataTable tjTbl = CommonBase.GetTable(sql);
                if(tjTbl.Rows.Count > 0)
                {
                    checkPoint(member, tjTbl.Rows[0]["MCode"].ToString(), listComm, ref rank);
        ...
        public static void checkPoint(Model.Member member, string MCode, List<CommonObject> listComm, ref M_Rank refNewRank)
        {
            ...
            if(tjRankTbl.Rows.Count > 0)
            {
                ...
                //update by zhuxy at 2018年4月21日22:25:08：重新修改的排位逻辑
                //从跟节点查询这棵树的所有叶子节点
                string sqlAallLeavels = "SELECT * FROM dbo.[FUN_CountTDMemberWithRank]('" + rank.MCode + "',1,9999)";
                DataTable tbAllTreeLeavels = CommonBase.GetTable(sqlAallLeavels);
                //最后一个节点（按照排位时间顺序最新的那条排位）
                DataRow lastRankRow = tbAllTreeLeavels.Select(string.Empty, "RankTime DESC")[0];
                //已经排位的最近的一个节点
                M_Rank lastRankModel = listAllRank.FirstOrDefault(c => c.Code == lastRankRow["Code"].ToString());

                //查找出最大的层级
                int maxLeavel = MethodHelper.ConvertHelper.ToInt32(tbAllTreeLeavels.Compute("MAX(LEAVEL)", string.Empty), 0);
                //最大层级-1就是要循环的节点
                //获取到这一行的所有节点，取出来这一排的所有节点
                DataRow[] allToRankLeavel = tbAllTreeLeavels.Select("LEAVEL=" + (maxLeavel - 1).ToString(), "RankTime ASC");
                //排位数量
                int rankCount = int.Parse(MethodHelper.ConfigHelper.GetAppSettings("RankCount"));//这里要从数据库中读取
                //循环这一排
                bool isBreak = false, isRanked = false;
                for(int i = 1; i <= rankCount; i++)
                {
                    foreach(DataRow dr in allToRankLeavel)
                    {
                        DataRow[] mbdArray = tbAllTreeLeavels.Select("P_ID='" + dr["ID"].ToString() + "' and MBD=" + i);
                        if(mbdArray.Length <= 0)
                        {
                            #region  该节点下该区位没有会员，就把该会员排到这里
                            M_Rank newRank = new M_Rank();
                            newRank.Code = MethodHelper.CommonHelper.GetGuid;
                            newRank.IsDeleted = false;
                            newRank.Leavel = MethodHelper.ConvertHelper.ToInt32(dr["LEAVEL"].ToString(), 0) + 1;
                            newRank.MBD = i;
                            newRank.MCode = member.ID;
                            newRank.PMCode = dr["ID"].ToString();
                            newRank.RankTime = DateTime.Now;
                            newRank.Sort = 1;
                            newRank.Status = 1;
                            CommonBase.Insert<M_Rank>(newRank, listComm);
                ...
                //如果没有排序过，就是需要新的一行进行排列，找到现在最后一排的第一个人作为父节点，进行新的排位
                if(!isRanked)
                {
                    DataRow dr = tbAllTreeLeavels.Select("LEAVEL=" + (maxLeavel).ToString(), "RankTime ASC")[0];
                    M_Rank newRank = new M_Rank();
                    newRank.Code = MethodHelper.CommonHelper.GetGuid;
                    newRank.IsDeleted = false;
                    newRank.Leavel = MethodHelper.ConvertHelper.ToInt32(dr["LEAVEL"].ToString(), 0) + 1;
        ...
        /// <summary>
        /// 记无忧树形结构往上发代言红包，以三个学员为单位向上发放奖励
        /// //第一个学员加入，上1，2，3级推荐人分别获得100元红包
        /// /// 第二个学员加入，上4，5，6级推荐人分别获得100元红包
        /// /// 第三个学员加入，上7，8，9级推荐人分别获得100元红包
        /// </summary>
        /// <param name="member"></param>
        /// <param name="money"></param>
        /// <param name="tjTbl"></param>
        /// <param name="listComm"></param>
        public static void UpFHChangeMoney(Model.Member member, decimal money, DataTable tjTbl, List<CommonObject> listComm, out List<TD_FHLog> listFHLog)
        {
            List<TD_FHLog> fhListLog = new List<TD_FHLog>();
            if(tjTbl != null && tjTbl.Rows.Count > 0)
            {
                //这个树形Table的最后一行就是树形结构的根节点
                string rootMemberCode = tjTbl.Rows[tjTbl.Rows.Count - 1]["ID"].ToString();

                string rootFromMemberCode = tjTbl.Rows[0]["ID"].ToString();

                //查到这个树
                //从跟节点查询这棵树的所有叶子节点
                //string sqlAallLeavels = "SELECT * FROM dbo.[FUN_CountTDMemberWithRank]('" + rootMemberCode + "',1,9999)";
                //2019年3月1日16:20:08新调整
                string sqlAallLeavels = "SELECT * FROM dbo.[FUN_CountTDMemberWithRank]('" + rootFromMemberCode + "',0,9999) where MRANK=1";
                DataTable tbAllTreeLeavels = CommonBase.GetTable(sqlAallLeavels);
                //查看里面现在有多少人，目前就是第几人加入
                int joinCount = tbAllTreeLeavels.Rows.Count;
                //查看是给123，456，789的哪一 级发红包
                int less3 = joinCount % 3;
                string upLeavels = "";
                switch(less3)
                {
                    case 1: upLeavels = "1,2,3"; break;
                    case 2: upLeavels = "4,5,6"; break;
                    case 0: upLeavels = "7,8,9"; break;
                }
                //查找到
                DataRow[] upLeavelMemberRows = tjTbl.Select("MRANK IN (" + upLeavels + ") ");
                //查找到每一级分红金额
                List<TD_SHMoney> shMoneyList = CacheService.SHMoneyList.Where(c => c.Code == "RankLeavel").ToList();
                foreach(DataRow row in upLeavelMemberRows)
                {
                    //if (row["IsFH"].ToString() == "1")
                    {
                        TD_SHMoney shmoney = shMoneyList.FirstOrDefault(c => c.TJIndex == Convert.ToInt16(row["MRANK"].ToString()));
                        if(shmoney != null)
                        {
                            #region 添加分红记录
                            decimal fhMoney = shmoney.TJFloat * money;
                            ...
                            string fh_remark = DateTime.Now.ToString() + "系统随机发放" + fhMoney + "元代言红包";
